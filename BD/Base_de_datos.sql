...
employee (id, firstname, lastname, department_id, salary, educationlevel_id) 
department (id, department_name, department_city) 
educationlevel (id, description)

select nombredepartamento, count(e.id) 
from departamento 
join empleado on e.departamentoid = departamento id
group by  deparamento_id
having count(e.id) >=2;
